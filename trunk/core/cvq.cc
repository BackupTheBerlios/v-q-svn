 ...
 * \return err_no if values were correct
 */
uint8_t cvq::qt_parse( const string & str, 
	...
	do {
			is>>qv;
			is>>ch;
			if(is.fail()) return lr(err_temp,"Invalid quota");
			switch(ch) {
			case 'C': 
					files = qv;
					break;
			case 'S': 
					...
					break;
			default:
					return lr(err_temp, "Invalid quota");
			}
			is>>ch;
			if(is.eof()) break;
			if(ch != ',') return lr(err_temp,"Invalid quota");
	} while( ! is.eof() && ! is.fail() );
	
	return is.eof() ? lr(err_no,"") : lr(err_temp,"Invalid quota");
}
