...
static CosNaming::NamingContext_ptr contactNs(CORBA::ORB_ptr orb)
	...
	CORBA::Object_ptr		obj) throw(ImportExportException)
{
	...
	//--------
	ns_obj = contactNs(orb);

	//--------
	// Convert "NameService#..." into CosNaming::Name format
	//--------
	name = NsStringToName(instructions + strlen(ns_prefix));
	if (name->length() == 0) {
...
contactNs(CORBA::ORB_ptr orb) throw(ImportExportException)
{
	CosNaming::NamingContext_var	ns_obj;
	CORBA::Object_var		obj;
	...
	try {
		...
		strstream	out;
		out	<< "resolve_initial_references(\"NameService\") "
			<< "failed: "
			<< ex
			<< ends;
		throw ImportExportException(out);
