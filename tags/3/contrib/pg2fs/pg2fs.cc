...
bool dnsc_chg(const string &host, const string &ns_ip) {
  string hf = ac_dnsc_root.val_str()+"/servers/"+host;
  string hftmp = hf+".tmp";
  opfstream out( hftmp.c_str(), ios::trunc );
  if( ! out ) {
    cerr<<me<<": can't open: "<<hftmp<<": "<<strerror(errno)<<endl;
    return false;
  }
  if( chown( hftmp.c_str(), ac_dnsc_uid.val_int(), ac_dnsc_gid.val_int()) ) {
    cerr<<me<<": can't chown: "<<hftmp<<": "<<strerror(errno)<<endl;
    return false;
  }
  if( chmod( hftmp.c_str(), ac_dnsc_mode.val_int()) ) {
 ...
 */
void tdns_chg() {
  ipfstream hosts(tdns[0]), alis(tdns[1]), alis1(tdns[2]), servers(tdns[3]);
  if(!hosts) {
  ...
  string datatmp = ac_tdns_root.val_str()+"/data.pg2fs.tmp";
  opfstream out(datatmp.c_str(), ios::trunc );
  if( ! out ) {
    cerr<<me<<": can't open: "<< datatmp <<": "<<strerror(errno)<<endl;
    return;
  }
  if( chown(datatmp.c_str(), ac_tdns_uid.val_int(), ac_tdns_gid.val_int()) ) {
    cerr<<me<<": can't chown: "<< datatmp <<": "<<strerror(errno)<<endl;
    return;
  }
  if( chmod(datatmp.c_str(), ac_tdns_mode.val_int()) ) {
  ...
  string data = ac_tdns_root.val_str()+"/data.pg2fs";
  ...
  int ret;
  ret = system(ac_tdns_make.val_str().c_str());
  if( ret == -1 ) {
    cerr<<me<<": system==-1: "<<strerror(errno)<<endl;
    return;
  } else if( WIFEXITED(ret) && WEXITSTATUS(ret) ) {
    cerr<<me<<": cmd: "<<ac_tdns_make.val_str()<<": returned: "<<WEXITSTATUS(ret)<<endl;
  } else if( WIFSIGNALED(ret) ) {
    cerr<<me<<": cmd: "<<ac_tdns_make.val_str()<<": signal: "<<WTERMSIG(ret)<<endl;
    return;
  }
  ret = system(ac_dnsc_restart.val_str().c_str());
  if( ret == -1 ) {
    cerr<<me<<": system==-1: "<<strerror(errno)<<endl;
    return;
  } else if( WIFEXITED(ret) && WEXITSTATUS(ret) ) {
    cerr<<me<<": cmd: "<<ac_dnsc_restart.val_str()<<": returned: "<<WEXITSTATUS(ret)<<endl;
  } else if( WIFSIGNALED(ret) ) {
    cerr<<me<<": cmd: "<<ac_dnsc_restart.val_str()<<": signal: "<<WTERMSIG(ret)<<endl;
    return;
  }
}
/*
...
int main(int ac, char **av) {
  ...
  opfstream lock("lock", ios::trunc);
  if( chmod("lock", 0600) ) {
    cerr<<*av<<": chmod: lock: "<<strerror(errno)<<endl;
    return 111;
  }
  if( lock_exnb(lock.rdbuf()->fd()) ) {
