...
#define c_vq(x) c((conf_home+"/share/vqwww").c_str(),x,x,0644,getuid(),getgid(),false)
    c_vq("cftpmod.xslt");
...
#undef c_vq
#define c_vq(x) c((conf_home+"/libexec/vqwww").c_str(),x,x,0644,getuid(),getgid())
    c_vq("libhosting.so");
#undef c_vq
#define c_vq(x) c((conf_home+"/bin").c_str(),x,x,04755,getuid(),getgid())
    c_vq("wwwalichg");
#undef c_vq
    return 0;
}
