...
#define c_vq(x,m) c(x,((string)"bin/"+x).c_str(),m,ac_vq_uid.val_int(),ac_vq_gid.val_int())
	...
	c_vq("deliver",0555);
	c_vq("autoresp",0555);
	c_vq("libpgsqlauthd.so",0555);
	c_vq("libpgsqllogd.so",0555);
	c_vq("udot_ls",0555);
	c_vq("udot_add",0555);
	c_vq("udot_rm",0555);
...
#define c_vq(x) c(x,((string)"lib/"+x).c_str(),0644,ac_vq_uid.val_int(),ac_vq_gid.val_int())
	...
	c_vq("vq_conf.a");
#undef c_vq
	sl("libsplit.so.5.0", "lib/libsplit.so", true);
	sl("libfd.so.5.0", "lib/libfd.so", true);
	sl("libvq.so.5.0", "lib/libvq.so", true);
	sl("libfs.so.5.0", "lib/libfs.so" , true);
	sl("libsys.so.5.0", "lib/libsys.so", true);
	sl("liblog.so.5.0", "lib/liblog.so", true);
	sl("libtext.so.5.0", "lib/libtext.so", true);
